...
class ChiselBackbone(nn.Module):
    ...
    def forward(self, x, skip_attention=False):
        ...
        if self.attention_layers is not None and not skip_attention:
            ...
            # No attention layers or skip_attention requested
...
def greedy_domain_assignment(A_hat, K_init, device='cpu', N_iter=3,
    ...
    for n in range(batchsize):
        ...
        L = A_hat_n.shape[0]
        V = torch.zeros((L, K_init), device=device)

        loss_val = mse_loss(V, A_hat_n)

        for _ in range(N_iter):
            for j in range(L):
                # Compute loss contribution of current V[j]
                v_j = V[j]
                a_j = A_hat_n[j]
                a_jj = a_j[j]
                ...
                if v_j.any():
                    prod = v_j @ V.T  # (L,)
                    mse_j = 2 * ((prod - a_j) ** 2).sum() - (1 - a_jj) ** 2
                else:
                    mse_j = 2 * (a_j ** 2).sum() - a_jj ** 2

                loss_minus_d = loss_val - mse_j
                V[j] = 0

                # Null loss (V[j] stays 0)
                L0 = loss_minus_d + 2 * (a_j ** 2).sum() - a_jj ** 2

                # Vectorized: compute loss for ALL K assignments at once
                # For V[j] = e_k: mse = 2*S_k - (1-a_jj)^2
                # where S_k = sum_i((V_col_k_with_j=1 - a_j)^2)
                diff_sq_sum = ((V - a_j.unsqueeze(1)) ** 2).sum(dim=0)  # (K_max,)
                L_opt = (loss_minus_d + 2 * diff_sq_sum
                         - 2 * a_jj ** 2 + (1 - a_jj) ** 2)

                z = torch.argmin(L_opt)
                if L_opt[z] < L0:
                    V[j, z] = 1
                    loss_val = L_opt[z].item()
                else:
                    loss_val = L0.item()
